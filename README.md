...
Exact member emails and D1-only E.164 contacts/consent/suppression state, household timezone and optional reminder-time overrides, deployment domain, Google credentials, Better Auth secret, and Cloudflare D1 binding are operator-supplied deployment inputs. Textbelt uses the fixed public key `textbelt`; there is no reminder-provider secret. See the [security contract](docs/operations/security.md), [operator runbook](docs/operations/operator-runbook.md), and [acceptance-evidence matrix](docs/operations/mvp-acceptance-evidence.md).
...
Use `npm run operator:contacts:prepare` with mode-`0600` input/output paths outside
the repository to prepare exact emails and E.164/consent/suppression updates. The
tool prints category counts, not values. Apply the generated private SQL only after
migrations `0001`–`0008`; then follow the runbook's redacted local/remote checks.
