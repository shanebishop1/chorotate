...
- Keep D1 as the only assignment/contact/reminder authority. Do not log OAuth tokens, cookies, full phone numbers, provider payloads, or raw provider failures.
- `npm run operator:contacts:prepare` accepts only mode-restricted files outside the repository, writes a new mode-`0600` D1 SQL file without overwrite, and reports contact readiness only as category counts. `npm run operator:d1:verify` captures remote JSON and reports check names without contact, identity, receipt, or resource values.
