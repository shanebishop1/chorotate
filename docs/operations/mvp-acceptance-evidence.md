...
| 11. Automated coverage includes chore periods, E.164/suppression, Textbelt segment/quota/status/timeout behavior, accessibility, and responsive checks. | `npm run check:release` runs doctor, formatting/lint/type generation, the complete Node and workerd/D1 suites, planning checks, private bootstrap/contact tests, redacted exact-structure D1 dry-run, production build, the complete Playwright phone/desktop/light/dark matrix with Axe/keyboard/320px assertions, dependency audit, secret scan, and tracked-file drift detection. Runtime tests additionally prove disabled production Cron returns before planning or Textbelt construction. | **Automated credential-free coverage**; provider/deploy checks remain external. |
...
- The 2026-09-01 production-handoff convergence run passed 156 Node tests, 15 workerd/D1 tests, 8 private bootstrap/contact/verification-contract tests, 60 Playwright journeys, production build and dry-run, doctor, quality contracts, dependency audit, secret scan, and the non-sending remote-D1 command dry-run.
