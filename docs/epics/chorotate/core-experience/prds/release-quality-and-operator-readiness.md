...
- T-1.1: Establish non-mutating CI, doctor, quality, security, contact-redaction, and removed-Resend checks. DoD: A clean checkout runs the complete quality/security surface without tracked-file mutation or secret/contact exposure, contains no Resend config/transport claim, and treats Textbelt public key as non-secret fixed configuration.
