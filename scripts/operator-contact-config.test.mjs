...
test("an exact-email change revokes sessions and releases only that auth binding", () => {
  ...
  database.exec(`
    INSERT INTO "user" (id,name,email,emailVerified,createdAt,updatedAt) VALUES
      ('auth-jack','Jack','jack@example.com',1,'2026-09-01','2026-09-01'),
      ('auth-joe','Joe','joe@example.com',1,'2026-09-01','2026-09-01');
    UPDATE allowlisted_identities SET auth_user_id = 'auth-jack' WHERE member_id = 'jack';
    UPDATE allowlisted_identities SET auth_user_id = 'auth-joe' WHERE member_id = 'joe';
    INSERT INTO "session" (id,expiresAt,token,createdAt,updatedAt,userId) VALUES
      ('session-jack','2026-10-01','token-jack','2026-09-01','2026-09-01','auth-jack'),
      ('session-joe','2026-10-01','token-joe','2026-09-01','2026-09-01','auth-joe');
  `);
  const changed = structuredClone(validInput);
  const jack = changed.members.find((member) => member.id === "jack");
  assert.ok(jack);
  ...
  assert.deepEqual(
    database
      .prepare(
        `SELECT member_id,email_normalized,auth_user_id
         FROM allowlisted_identities WHERE member_id IN ('jack','joe')
         ORDER BY member_id`,
      )
      .all()
      .map((row) => ({ ...row })),
    [
      {
        member_id: "jack",
        email_normalized: "jack.new@example.com",
        auth_user_id: null,
      },
      {
        member_id: "joe",
        email_normalized: "joe@example.com",
        auth_user_id: "auth-joe",
      },
    ],
  );
  assert.deepEqual(
