...
describe("authenticated home route integration", () => {
  ...
  it("materializes from configuration membership snapshots after deactivation", async () => {
    const fixture = database();
    fixture.sqlite.exec(
      "DROP TRIGGER weekly_assignments_no_delete; DELETE FROM weekly_assignments; UPDATE members SET active=0 WHERE id='joe'",
    );
  ...
  it("returns current values on a stale conflict without overwriting", async () => {
    ...
    expect(stale).toMatchObject({
      state: "conflict",
      current: [{ memberId: "joe", version: 2 }],
    });
    expect(
      fixture.sqlite
        .prepare(
